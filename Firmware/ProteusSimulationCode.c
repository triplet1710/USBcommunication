...
void main() 
{
    ...
    prev_state_led0 = 1;
    if(prev_state_led0 && Button(&PORTD, 4, 2, 0)) // Doc canh xuong nut nhan
    {
       nutnhan[0] = '1'; xacnhan1 = 1;
       prev_state_led0 = 0;
    }
    else {
    nutnhan[0] = '0'; xacnhan1 = 0; // xacnhan = 0 -> khong nhan nut
    }
    ...
    writebuff[0] = '&'; //nhan gia tri tu nut nhan
    ...
    if (HID_Read() != 0)
    {
    if (readbuff[0] == 'B')//Doc nut nhan tren GUI
    {
       if (readbuff[1] == '@') LATD0_bit = ~LATD0_bit;// port D0 = 0 -> den sang
       if (!LATD0_bit) writebuff[1] = 'S';
       else writebuff[1] = 'T';
       
       if (readbuff[2] == '@') LATD1_bit = ~LATD1_bit;// port D1 = 0 -> den sang
       if (!LATD1_bit) writebuff[2] = 'S';
       else writebuff[2] = 'T';

       if (readbuff[3] == '@') LATD2_bit = ~LATD2_bit;// port D2 = 0 -> den sang
       if (!LATD2_bit) writebuff[3] = 'S';
       else writebuff[3] = 'T';

       if (readbuff[4] == '@') LATD3_bit = ~LATD3_bit; // port D3 = 0 -> den sang
